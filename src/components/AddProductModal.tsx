...
// B1: เพิ่ม Type 'Tag' เข้าไปใน import เดิม
...
        }).catch(console.error);
        ...
        const fetchProductTags = async () => {
          const { data, error } = await supabase
            .from('product_tags').select('tags(name)').eq('product_id', editingProduct.id);
          if (error) {
            console.error("Error fetching product tags:", error);
            setSelectedTags([]);
          } else {
            const currentTags = (data || []).map(pt => pt.tags ? { value: pt.tags.name, label: pt.tags.name } : null).filter(Boolean) as { value: string; label: string }[];
...
          <DialogHeader>
            <DialogTitle className="text-xl text-purple-800">{editingProduct ? "แก้ไขสินค้า" : "+ เพิ่มสินค้าใหม่"}</DialogTitle>
          </DialogHeader>

          {/* --- JSX เดิมของคุณทั้งหมด --- */}
...
            />
            ...
            {/* --- B7: แทรก UI ของ Tag เข้ามาตรงนี้ --- */}
            <Card>
